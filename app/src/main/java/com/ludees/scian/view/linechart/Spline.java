...
public class Spline {
    ...
    public Spline(double xx[], double ff[]) {
        ...
        if (n > 3) fpn = fpn + b[n - 2]
                         * (c[n - 2] - c[n - 3] - (b[n - 3] + b[n - 2]) * (c[n - 3] - c[n - 4]) / (b[n - 3] + b[n - 4]))
                         / (x[n - 1] - x[n - 4]);
    ...
    public double spline_value(double t) {
        ...
        if (t > x[n - 2]) interval = n - 2;
        else if (t >= x[last_interval]) for (int j = last_interval; j < n && t >= x[j]; j++)
            interval = j;
        else for (int j = last_interval; t < x[j]; j--)
            interval = j - 1;
