package com.ludees.scian.view.linechart;
...
import com.ludees.scian.view.linechart.LinePoint.Type;
...
public class LineChartView extends View {
	...
	private float mViewPortMarginLeft, mViewPortMarginRight,
			mViewPortMarginTop, mViewPortMarginBottom;
	...
	// private List<Line> myLine = null;
	private Bitmap cropBitmap = null;
	private Bitmap cropHorValuesBitmap = null;
	private Bitmap b;
	...
	/**
	 * The listener interface for receiving onChartPointClick events. The class
	 * that is interested in processing a onPointClick event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's <code>setOnPointClickListener</code>
	 * method. When the onPointClick event occurs, that object's appropriate
	 * method is invoked.
	 */
	public interface OnChartPointClickListener {
		 ...
		 * 
		 * @param point
		 *            the point nearest to clicked position in point click
		 *            radius
		 * @param line
	...
	private void setAttr(Context context, AttributeSet attrs) {
		...
		mGrid.stepVer = a.getInt(
				R.styleable.LineChartView_horizontal_grid_step, mGrid.stepVer);
		mGrid.stepHor = a.getInt(R.styleable.LineChartView_vertical_grid_step,
				mGrid.stepHor);
		mGrid.horSubLinesCount = a.getInt(
				R.styleable.LineChartView_horizontal_grid_sublines_count,
				mGrid.horSubLinesCount);
		mGrid.verSubLinesCount = a.getInt(
				R.styleable.LineChartView_vertical_grid_sublines_count,
				mGrid.verSubLinesCount);
		mGrid.verMainLinesEnabled = a.getBoolean(
				R.styleable.LineChartView_vertical_grid,
				mGrid.verMainLinesEnabled);
		mGrid.verSubLinesEnabled = a.getBoolean(
				R.styleable.LineChartView_vertical_grid_sublines,
				mGrid.verSubLinesEnabled);
		mGrid.horMainLinesEnabled = a.getBoolean(
				R.styleable.LineChartView_horizontal_grid,
				mGrid.horMainLinesEnabled);
		mGrid.horSubLinesEnabled = a.getBoolean(
				R.styleable.LineChartView_horizontal_grid_sublines,
				mGrid.horSubLinesEnabled);
		color = a.getColor(R.styleable.LineChartView_horizontal_grid_color,
				mGrid.mainHorLinesPaint.getColor());
		mGrid.mainHorLinesPaint.setColor(color);
		color = a.getColor(R.styleable.LineChartView_vertical_grid_color,
				mGrid.mainVerLinesPaint.getColor());
		mGrid.mainVerLinesPaint.setColor(color);
		color = a.getColor(
				R.styleable.LineChartView_horizontal_grid_sublines_color,
				mGrid.subHorLinesPaint.getColor());
		mGrid.subHorLinesPaint.setColor(color);
		color = a.getColor(
				R.styleable.LineChartView_vertical_grid_sublines_color,
				mGrid.subVerLinesPaint.getColor());
		mGrid.subVerLinesPaint.setColor(color);
		// grid thikness
		dim = a.getDimension(
				R.styleable.LineChartView_horizontal_grid_thikness,
				mGrid.mainHorLinesPaint.getStrokeWidth());
		mGrid.mainHorLinesPaint.setStrokeWidth(dim);
		dim = a.getDimension(R.styleable.LineChartView_vertical_grid_thikness,
				mGrid.mainVerLinesPaint.getStrokeWidth());
		mGrid.mainVerLinesPaint.setStrokeWidth(dim);
		dim = a.getDimension(
				R.styleable.LineChartView_horizontal_grid_sublines_thikness,
				mGrid.subHorLinesPaint.getStrokeWidth());
		mGrid.subHorLinesPaint.setStrokeWidth(dim);
		dim = a.getDimension(
				R.styleable.LineChartView_vertical_grid_sublines_thikness,
				mGrid.subVerLinesPaint.getStrokeWidth());
		mGrid.subVerLinesPaint.setStrokeWidth(dim);
		// values color
		color = a.getColor(R.styleable.LineChartView_horizontal_values_color,
				mGrid.mainHorValuesPaint.getColor());
		mGrid.mainHorValuesPaint.setColor(color);
		color = a.getColor(R.styleable.LineChartView_vertical_values_color,
				mGrid.mainVerValuesPaint.getColor());
		mGrid.mainVerValuesPaint.setColor(color);

		dim = a.getDimension(R.styleable.LineChartView_horizontal_values_size,
				mGrid.mainHorValuesPaint.getTextSize());
		mGrid.mainHorValuesPaint.setTextSize(dim);
		dim = a.getDimension(R.styleable.LineChartView_vertical_values_size,
				mGrid.mainVerValuesPaint.getTextSize());
		mGrid.mainVerValuesPaint.setTextSize(dim);
		mGrid.horMainValuesEnabled = a.getBoolean(
				R.styleable.LineChartView_horizontal_values,
				mGrid.horMainValuesEnabled);
		mGrid.verMainValuesEnabled = a.getBoolean(
				R.styleable.LineChartView_vertical_values,
				mGrid.verMainValuesEnabled);
		a.recycle();
	}
	...
	/**
	 * Sets the additional movement for ViewPort after limits Max and Min values
	 * of LinePoints.
	 * 
	 * @param hor
	...
	/**
	 * Sets the vertical grid lines style using {@link android.graphics.Paint
	 * Paint} for drawing
	 * 
	 * @param main
	...
	public void setVerticalGridStyle(Paint main, Paint sub) {
		if (main != null)
			mGrid.mainVerLinesPaint = main;
		if (sub != null)
			mGrid.subVerLinesPaint = sub;
	}

	/**
	 * Sets the horizontal grid lines style using {@link android.graphics.Paint
	 * Paint} for drawing
	 * 
	 * @param main
	 *            style for main vertical grid lines
	...
	public void setHorizontalGridStyle(Paint main, Paint sub) {
		if (main != null)
			mGrid.mainHorLinesPaint = main;
		if (sub != null)
			mGrid.subHorLinesPaint = sub;
	}
	 ...
	 * @param sub
	 *            - is additional vertical grid lines visible? (Not visible
	 *            without main lines)
	 * 
	 * @see #setVerticalGridStyle
	 */
	 ...
	 * @param sub
	 *            - is additional horizontal grid lines visible? (Not visible
	 *            without main lines)
	 * 
	 * @see #setHorizontalGridStyle
	 */
	 ...
	 * @see #enableVerticalGrid
	 */
	public void setGridSize(int horStep, int horSubLinesCount, int verStep,
	...
	public void setMainValuesStyle(Paint hor, Paint ver) {
		if (hor != null)
			mGrid.mainHorValuesPaint = hor;
		if (ver != null)
			mGrid.mainVerValuesPaint = ver;
	}
	...
	public void setHorValuesMarginsDP(int left, int bottom, int right, int top) {
		mGrid.horValuesMarginLeft = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, left, getResources()
						.getDisplayMetrics());
		mGrid.horValuesMarginRight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, right, getResources()
						.getDisplayMetrics());
		mGrid.horValuesMarginTop = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, top, getResources()
						.getDisplayMetrics());
		mGrid.horValuesMarginBottom = (int) TypedValue.applyDimension(
	...
	public void setVerValuesMarginsDP(int left, int bottom, int right, int top) {
		mGrid.verValuesMarginLeft = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, left, getResources()
						.getDisplayMetrics());
		mGrid.verValuesMarginRight = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, right, getResources()
						.getDisplayMetrics());
		mGrid.verValuesMarginTop = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, top, getResources()
						.getDisplayMetrics());
		mGrid.verValuesMarginBottom = (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, bottom, getResources()
						.getDisplayMetrics());
		cropVerValuesShader = null;
	}
	...
	/**
	 * Sets custom text for horizontal values. Use
	 * {@link android.util.SparseArray} for mapping grid value and text for it.
	 * If no text for value, grid line will drawn without text.
	 * 
	 * @param map
	 *            {@link android.util.SparseArray} with text for drawing. If
	 *            <i>map</i> is not null text for lines taking from it. Else
	 *            integer values from grid will be drawn.
	...
	/**
	 * Sets custom text for vertical values. Use
	 * {@link android.util.SparseArray} for mapping grid value and text for it.
	 * If no text for value, grid line will drawn without text.
	 * 
	 * @param map
	 ...
	 */
	public void setViewPortMargins(float left, float bottom, float right,
			float top) {
	 ...
	 * @see #setViewPortMargins
	 */
	public void setViewPortMarginsDP(float left, float bottom, float right,
			float top) {
		mViewPortMarginLeft = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, left, getResources()
						.getDisplayMetrics());
		mViewPortMarginRight = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, right, getResources()
						.getDisplayMetrics());
		mViewPortMarginTop = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, top, getResources()
						.getDisplayMetrics());
		mViewPortMarginBottom = TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, bottom, getResources()
						.getDisplayMetrics());
		cropViewPortShader = null;
		scaleCorrection();
	...
	private void scaleCorrection() {
		mScaleX = (mViewPortRight - mViewPortLeft != 0) ? (getMeasuredWidth()
				- mViewPortMarginLeft - mViewPortMarginRight)
				/ (mViewPortRight - mViewPortLeft) : 1;
		mScaleY = (mViewPortTop - mViewPortBottom != 0) ? (getMeasuredHeight()
				- mViewPortMarginTop - mViewPortMarginBottom)
				/ (mViewPortTop - mViewPortBottom) : 1;
	}
	...
	public void onDraw(Canvas canvas) {
		...
		if (cropViewPortShader == null) {
			...
			if (cropBitmap == null) {
				num++;
				cropBitmap = Bitmap.createBitmap(getMeasuredWidth(),
						getMeasuredHeight(), Config.ALPHA_8);
				System.out.println("AAAAAAAAAAAAAA------------>>>>>>>>" + num);
			}
			...
			cropCanvas.drawRect(mViewPortMarginLeft - 1,
					mViewPortMarginTop - 1, getWidth() - mViewPortMarginRight
							+ 1 + mGrid.stepHor * mScaleY, getHeight()
							- mViewPortMarginBottom + 1, cropPaint);
			cropViewPortShader = new BitmapShader(cropBitmap,
	...
	private void drawLine(Canvas canvas, Line line) {
		...
		if (line.isFilled()) {
			...
			mMatrix.setTranslate(
					-mViewPortLeft * mScaleX + mViewPortMarginLeft,
					(mViewPortTop) * mScaleY + mViewPortMarginTop);
			pathCopy.transform(mMatrix);
		...
		pathCopy.transform(mMatrix);
		mMatrix.reset();
		mMatrix.setTranslate(-mViewPortLeft * mScaleX + mViewPortMarginLeft,
				(mViewPortTop) * mScaleY + mViewPortMarginTop);
		pathCopy.transform(mMatrix);
	...
	private void drawPoint(Canvas canvas, LinePoint point) {
		if (point.isVisible()) {
			float x = point.getX() * mScaleX - mViewPortLeft * mScaleX
					+ mViewPortMarginLeft;
			float y = point.getY() * (-mScaleY) + mViewPortTop * mScaleY
					+ mViewPortMarginTop;
			if (x
					+ (point.getRadius() + point.getStrokePaint()
							.getStrokeWidth()) > mViewPortMarginLeft
					&& x
							- (point.getRadius() + point.getStrokePaint()
									.getStrokeWidth()) < getWidth()
							- mViewPortMarginRight
					&& y
							+ (point.getRadius() + point.getStrokePaint()
									.getStrokeWidth()) > mViewPortMarginTop
					&& y
							- (point.getRadius() + point.getStrokePaint()
									.getStrokeWidth()) < getHeight()
				...
				if (point.getType() == Type.CIRCLE) {
					canvas.drawCircle(x, y, point.getRadius(),
							point.getFillPaint());
					canvas.drawCircle(x, y, point.getRadius(),
							point.getStrokePaint());
				} else if (point.getType() == Type.SQUARE) {
					canvas.drawRect(x - point.getRadius(),
							y - point.getRadius(), x + point.getRadius(), y
									+ point.getRadius(), point.getFillPaint());
					canvas.drawRect(x - point.getRadius(),
							y - point.getRadius(), x + point.getRadius(), y
									+ point.getRadius(), point.getStrokePaint());
					...
					path.lineTo(x - 0.86f * point.getRadius(),
							y + 0.5f * point.getRadius());
					path.lineTo(x + 0.86f * point.getRadius(),
							y + 0.5f * point.getRadius());
					path.close();
	...
	private void drawPointText(Canvas canvas, LinePoint point) {
		...
		if (point.isTextVisible()) {
			...
			if ((point.getTextAlign() & TextAlign.TOP) > 0) {
				...
				txtY = y + point.getRadius() + point.getTextPaint().descent()
						+ point.getTextPaint().getTextSize();
			}

			Paint mPaint = new Paint();
			mPaint.setTextAlign(Paint.Align.CENTER);
			mPaint.setColor(point.getStrokePaint().getColor());

			if (point.isDrawTextBackground() == true) {
				mPaint.setColor(point.getStrokePaint().getColor());
				canvas.drawRect(txtX - dip2px(context, 20),
						txtY - dip2px(context, 25), txtX + dip2px(context, 20),
						txtY, mPaint);
				point.getTextPaint().setColor(Color.WHITE);
			} else {
				mPaint.setColor(0x00000000);
				canvas.drawRect(txtX - dip2px(context, 15),
						txtY - dip2px(context, 15), txtX + dip2px(context, 15),
						txtY, mPaint);
			}
			canvas.save();
			canvas.drawText(point.getText(), txtX, txtY - 5,
					point.getTextPaint());

			// boolean flag = false;
			// String[] num = point.getText().split("/");
			...
			//
			// if (flag) {
			// canvas.drawRect(txtX - dip2px(context, 40),
			// txtY + dip2px(context, 25), txtX + dip2px(context, 40),
			// txtY + 2 * dip2px(context, 25), mPaint);
			// canvas.save();
			// point.getTextPaint().setColor(Color.WHITE);
			// canvas.drawText(point.getText(), txtX,
			// txtY + dip2px(context, 37), point.getTextPaint());
			// } else {
			// canvas.drawRect(txtX - dip2px(context, 40),
			// txtY - dip2px(context, 25), txtX + dip2px(context, 40),
			// txtY, mPaint);
	...
	private void drawVerticalGrid(Canvas canvas) {
		int firstLertLineX = (((int) mViewPortLeft) / mGrid.stepHor - 1)
				* mGrid.stepHor;
		...
		if (mGrid.verMainLinesEnabled) {
			for (int x = firstLertLineX; x < mViewPortRight
					+ (mViewPortMarginRight + 20) / mScaleY; x += mGrid.stepHor) {
				canvas.drawLine((x - mViewPortLeft) * mScaleX
						+ mViewPortMarginLeft, mViewPortMarginTop,
				...
				if (mGrid.verSubLinesEnabled) {
					for (int i = 1; i <= mGrid.horSubLinesCount; i++) {
						canvas.drawLine((x + i * subStep - mViewPortLeft)
								* mScaleX + mViewPortMarginLeft,
								mViewPortMarginTop,
								(x + i * subStep - mViewPortLeft) * mScaleX
										+ mViewPortMarginLeft, getHeight()
										- mViewPortMarginBottom,
	...
	private void drawHorizontalGrid(Canvas canvas) {
		int firstHorLineY = (((int) mViewPortBottom) / mGrid.stepVer)
				* mGrid.stepVer;
		...
		if (mGrid.horMainLinesEnabled) {
			for (int y = firstHorLineY; y <= mViewPortTop; y += mGrid.stepVer) {
				canvas.drawLine(mViewPortMarginLeft, getHeight()
						- mViewPortMarginBottom - (y - mViewPortBottom)
						* mScaleY, getWidth() - mViewPortMarginRight,
						getHeight() - mViewPortMarginBottom
								- (y - mViewPortBottom) * mScaleY,
					...
					for (int i = 1; i <= mGrid.verSubLinesCount; i++) {
						canvas.drawLine(mViewPortMarginLeft,
								getHeight() - mViewPortMarginBottom
										...
										* mScaleY, getWidth()
										- mViewPortMarginRight, getHeight()
										- mViewPortMarginBottom
										- (y - mViewPortBottom + i * subStep)
										* mScaleY, mGrid.subHorLinesPaint);
					}
				}
			}
		}
		...
		if (mGrid.horMainValuesEnabled) {
			if (cropHorValuesShader == null) {
				...
				if (cropHorValuesBitmap == null) {
					cropHorValuesBitmap = Bitmap
							.createBitmap(getWidth(),
									(int) (mGrid.mainHorValuesPaint
											.getTextSize() * 1.2f),
									Config.ALPHA_8);
				}
				...
				float descent = (mGrid.mainHorValuesPaint == null) ? 0
						: mGrid.mainHorValuesPaint.descent();

				cropCanvas.drawRect(mGrid.horValuesMarginLeft,
						mGrid.horValuesMarginTop, getWidth()
								- mGrid.horValuesMarginRight, getHeight()
								- mGrid.horValuesMarginBottom + descent,
						cropPaint);
				cropHorValuesShader = new BitmapShader(cropHorValuesBitmap,
						Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
			}
			int first = ((int) ((mViewPortLeft - mViewPortMarginLeft / mScaleX) / mGrid.stepHor) - 1)
					* mGrid.stepHor;
			int last = (int) (mViewPortRight + mViewPortMarginRight / mScaleX)
					+ mGrid.stepHor;
			...
			for (int x = first; x <= last; x += mGrid.stepHor) {
				...
				if (mGrid.horValuesText != null)
					string = mGrid.horValuesText.get(x);
				if (string == null)
					string = "";

				float txtY = getHeight() - 1 - mGrid.horValuesMarginBottom;
				float txtX = (x - mViewPortLeft) * mScaleX
						+ mViewPortMarginLeft;

				if ((mGrid.horValuesAlign & TextAlign.TOP) > 0) {
					txtY = mGrid.horValuesMarginTop
							+ mGrid.mainHorValuesPaint.getTextSize();
				} else if ((mGrid.horValuesAlign & TextAlign.VERTICAL_CENTER) > 0) {
					txtY = (getHeight()
							- (mGrid.horValuesMarginTop + mGrid.horValuesMarginBottom) + mGrid.mainHorValuesPaint
							.getTextSize()) / 2;
				}
				...
				// canvas.drawText(string, txtX, txtY,
				// mGrid.mainHorValuesPaint);
				// TODO
				if (string
						.matches("^[0-9]{1,2}[-][0-9]{1,2} [0-9]{1,2}:[0-9]{1,2}$")) {
					String[] split = string.split(" ");
					canvas.drawText(split[0], txtX, txtY,
							mGrid.mainHorValuesPaint);
					canvas.drawText(split[1], txtX, txtY
							+ mGrid.mainHorValuesPaint.getTextSize() * 1.2f,
							mGrid.mainHorValuesPaint);
				} else {
					canvas.drawText(string, txtX, txtY,
							mGrid.mainHorValuesPaint);
	...
	private void drawVerticalValues(Canvas canvas) {
		if (mGrid.verMainValuesEnabled) {
			if (cropVerValuesShader == null) {
				// TODO
				if (cropBitmap == null) {
					cropBitmap = Bitmap.createBitmap(getWidth(), getHeight(),
							Config.ALPHA_8);
				}
				...
				cropPaint.setColor(0xffffffff);
				cropPaint.setStyle(Paint.Style.FILL);
				float descent = (mGrid.mainVerValuesPaint == null) ? 0
						: mGrid.mainVerValuesPaint.descent();
				cropCanvas.drawRect(mGrid.verValuesMarginLeft,
						mGrid.verValuesMarginTop, getWidth()
								- mGrid.verValuesMarginRight, getHeight()
								- mGrid.verValuesMarginBottom + descent,
						cropPaint);
				cropVerValuesShader = new BitmapShader(cropBitmap,
						Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
			}
			int firstHorLineY = ((int) (mViewPortBottom - mViewPortMarginBottom
					/ mScaleY)
					/ mGrid.stepVer - 1)
					* mGrid.stepVer;
			int last = (int) (mViewPortTop + mViewPortMarginTop / mScaleY)
					+ mGrid.stepVer;
			// float subStep = (float) mGrid.stepVer / (mGrid.verSubLinesCount +
			// 1);
			mGrid.mainVerValuesPaint.setShader(cropVerValuesShader);
			for (int y = firstHorLineY; y < last; y += mGrid.stepVer) {
				String string = String.valueOf(y);
				if (mGrid.verValuesText != null)
					string = mGrid.verValuesText.get(y);
				if (string == null)
					string = "";
				int txtX = mGrid.verValuesMarginLeft;
				float txtY = (getHeight() - mViewPortMarginBottom)
						- (y - mViewPortBottom) * mScaleY
						- mGrid.mainVerValuesPaint.descent();

				if ((mGrid.verValuesAlign & TextAlign.TOP) > 0) {
					txtY = (getHeight() - mViewPortMarginBottom)
							- (y - mViewPortBottom) * mScaleY
							+ mGrid.mainVerValuesPaint.getTextSize()
							+ mGrid.mainVerValuesPaint.descent();
					...
					txtY = (getHeight() - mViewPortMarginBottom)
							- (y - mViewPortBottom)
							* mScaleY
							+ (mGrid.mainVerValuesPaint.getTextSize() - mGrid.mainVerValuesPaint
									.descent()) / 2;
				}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			if (Math.abs(downX - x) < mTouchSlop
					&& Math.abs(downY - y) < mTouchSlop) {
				if (mListener != null)
					findClickedVerticle(x, y);
			} else {
				frictionX = -Math.signum(velocityX)
						* Math.abs(mViewPortRight - mViewPortLeft) / 1000
						* mFrictionForceX;
				frictionY = -Math.signum(velocityY)
						* Math.abs(mViewPortTop - mViewPortBottom) / 1000
						* mFrictionForceY;
	...
	private void findClickedVerticle(float x, float y) {
		...
		float gY = (getHeight() - y - mViewPortMarginBottom) / mScaleY
				+ mViewPortBottom;
		...
		for (Line l : mLines)
			for (LinePoint p : l.getPoints()) {
				if (gX >= p.getX() - getPointClickRadius() / mScaleX
						&& gX <= p.getX() + getPointClickRadius() / mScaleX
						&& gY >= p.getY() - getPointClickRadius() / mScaleY
						&& gY <= p.getY() + getPointClickRadius() / mScaleY) {
					float r = (float) Math.sqrt((gX - p.getX())
							* (gX - p.getX()) + (gY - p.getY())
							* (gY - p.getY()));
					if (minR > r) {
						minR = r;
		...
		if (mListener != null && point != null)
			mListener.onPointClick(point, line);
	}
	...
	public void moveViewPort(float deltaX, float deltaY) {
		if (mViewPortLeft + deltaX > mMinX - mViewPortHorFreedom
				&& mViewPortRight + deltaX < mMaxX + mViewPortHorFreedom) {
		...
		if (mViewPortBottom - deltaY > mMinY - mViewPortVerFreedom
				&& mViewPortTop - deltaY < mMaxY + mViewPortVerFreedom) {
	...
	private void inertionMove() {
		if (isMovingX || isMovingY) {
			if (isMovingX) {
				...
				velocityX += frictionX;
				if (signX == (int) Math.signum(velocityX))
					moveViewPort(velocityX, 0);
				else
					isMovingX = false;
			}
			if (isMovingY) {
				...
				velocityY += frictionY;
				if (signY == (int) Math.signum(velocityY))
					moveViewPort(0, velocityY);
				else
					isMovingY = false;
			}
			postInvalidate();
	...
	public void delBitmap() {
		if (cropBitmap != null && getBitmapSize(cropBitmap) > 0) {
			cropBitmap.recycle();
		}
		...
		if (cropHorValuesBitmap != null
				&& getBitmapSize(cropHorValuesBitmap) > 0) {
			cropHorValuesBitmap.recycle();
		}
