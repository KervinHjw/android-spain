...
import com.ludees.scian.entity.YanZhengBean;
import com.ludees.scian.network.Urls;
import com.ludees.scian.view.TimeButton;
...
public class ForgetPwdActivity extends BaseActivity implements OnClickListener{
	...
	private void initData() {
		...
		finalHttp.addHeader("lord-app-language","English");
		...
		case R.id.pwd_btn_update:
			...
			if(mMail!=null){
				...
				if(!mMail.equals(mail.getText().toString())){
					dialog.dismiss();
					prompt(ForgetPwdActivity.this.getResources().getString(R.string.qqryxzq));
					return;
				}
			}
