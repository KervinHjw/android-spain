...
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        //文字的大小
        textSize = mTypedArray.getDimension(R.styleable.RoundProgressBar_textSize, 95);
        //圆环的宽度
        roundWidth = mTypedArray.getDimension(R.styleable.RoundProgressBar_roundWidth, 10);
        //最大进度
        max = mTypedArray.getInteger(R.styleable.RoundProgressBar_max, 300);
        //是否显示中间的进度
        ...
        if(textIsDisplayable && percent != 0 && style == STROKE){
        	if(flag){
        		if(down!=-1){
           		 ...
           		 canvas.drawText(String.valueOf(progress)+"/"+down, centre - textWidth / 2 - 20 , centre + textSize/2 - 50, paint); //画出进度百分比
           	}else{
           		 canvas.drawText(String.valueOf(progress), centre - textWidth / 2 + 20 , centre + textSize/2 - 50, paint); //画出进度百分比
           	}
        	}else{
            ...
            canvas.drawText(unit, centre - 80, centre + textSize/2 + 50, paint2); //画出进度百分比
        }
